...
def add_course(code,title):
    ...
    for course in data["courses"]:
        if course["code"] == code:
            raise ValueError("Course code already exist")
...
def enroll(student_id, course_code):
    ...
    if not  student_exists:
        raise ValueError("Student not fount")
    
    course_exists = any(course["code"] == course_code for course in data["courses"])
    if not course_exists:
         raise ValueError("Course not fount")
    
    already_enrolled = any(enrollment["student_id"] == student_id and enrollment["course_code"] == course_code for enrollment in data["enrollments"])
    if already_enrolled:
        raise ValueError("Student is already enrolled in thir course.")
...
def list_enrollments():
    ...
    data = load_data()
    # return sorted(data["enrollments"] , key=lambda enrollment: (enrollment["student_id"], enrollment["course_code"]))
    enrollments = [{
...
def compute_gpa(student_id):
    ...
    return sum(avarages) / len(avarages)
